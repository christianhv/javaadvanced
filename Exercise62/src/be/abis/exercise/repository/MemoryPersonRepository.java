...
public class MemoryPersonRepository implements PersonRepository {
    ...
    public Person findPersonByEmailAndPassword(String email, String password) throws PersonNotFoundException {
        Optional<Person> personFoundOptional = persons.stream()
                .filter(p->p.getEmail().equals(email) && p.getPassword().equals(password))
                .findFirst();
        Person foundPerson=null;
        if(personFoundOptional.isPresent()){
            infoLogger.info("Person logged in correctly.");
            foundPerson=personFoundOptional.get();
        } else {
            exceptionLogger.error("Combination email/password not correct.");
            throw new PersonNotFoundException("Combination email/password not correct.");
        }
    ...
    public List<Person> findPersonsForCompany(String companyName) throws PersonNotFoundException {
        ...
        if (foundPersons.size()==0) throw new PersonNotFoundException("No persons found for " + companyName + ".");
        return foundPersons;
    }
    ...
    public void addPerson(Person p) throws PersonAlreadyExistsException {
        ...
        } else throw new PersonAlreadyExistsException("Person with this email already in list.");
    }
}
